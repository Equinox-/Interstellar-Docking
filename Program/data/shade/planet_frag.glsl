...
void main() {
	...
	vec2 noiseVector = pow(atmPassed, 0.25) * 1E-6 * vec2(surface3(vertexPosition / 2), surface3(-vertexPosition / 2));
	
	vec4 diffuseColor = texture2D(diffuseMap, gl_TexCoord[0].st + noiseVector);
	vec4 specularColor = texture2D(specularMap, gl_TexCoord[0].st + noiseVector);
